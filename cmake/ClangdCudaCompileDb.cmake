...
# When clangd parses a header directly (without a source file context), it may
# fall back to a default compile command that lacks --cuda-path.  Inject it
# via the Add list so that CUDA builtins (threadIdx, blockIdx, etc.) are
# always resolvable.
if(CLANGD_CUDA_PATH)
    list(APPEND _clangd_cuda_stdlib_flags "    - --cuda-path=${CLANGD_CUDA_PATH}")
elseif(DEFINED ENV{CONDA_PREFIX})
    file(GLOB _clangd_cuda_target_dirs "$ENV{CONDA_PREFIX}/targets/*")
    if(_clangd_cuda_target_dirs)
        list(GET _clangd_cuda_target_dirs 0 _clangd_cuda_first_target)
        list(APPEND _clangd_cuda_stdlib_flags "    - --cuda-path=${_clangd_cuda_first_target}")
    endif()
endif()
...
if(CLANGD_CUDA_ENABLE_CLANGD_CONFIG)
    # Only the top-level project writes .clangd.  Subprojects (e.g. corelog
    # via FetchContent) may include their own copy of this module; letting
    # them write would overwrite the correct top-level configuration.
    if(CMAKE_CURRENT_SOURCE_DIR STREQUAL CMAKE_SOURCE_DIR)
        execute_process(COMMAND "${CMAKE_COMMAND}" -P "${_clangd_cuda_update_script}")
